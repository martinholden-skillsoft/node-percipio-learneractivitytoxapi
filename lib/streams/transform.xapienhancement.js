...
const xapienhancementTransformStream = (opts) => {
  ...
  const { xapilookup } = opts;
  const options = { ...defaults, xapilookup };
  ...
  if (fs.existsSync(options.xapilookup)) {
    ...
    throw new Error(
      `xapienhancementTransformStream. No such file or directory ${options.xapilookup}`
    );
  }

  const destroy = () => {
    options.xapilookupmap = null;
  };
